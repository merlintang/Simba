...
import org.apache.spark.sql.simba.index.{IndexType, IndexedRelation}
...
private[simba] class IndexManager extends Logging {
  ...
  private[simba] def persistIndex(simbaContext: SparkSession,
                                indexName: String,
  ...
  private[simba] def loadIndex(simbaContext: SparkSession, indexName: String, fileName: String): Unit = {
    val sparkContext = simbaContext.sparkContext
  ...
  private[simba] def showQuery(simbaContext: SparkSession, tableName: String): Unit = readLock {
    indexInfos.map(row => {
