...
import org.apache.spark.sql.simba.{IndexInfo, SparkSession}
import org.apache.spark.sql.simba.execution.join._
...
class SimbaPlanner(val simbaContext: SparkSession,
                   override val conf: SQLConf,
                   override val extraStrategies: Seq[Strategy]) extends SparkPlanner(simbaContext.sparkContext, conf, extraStrategies) {
  ...
  object SpatialJoinExtractor extends Strategy with PredicateHelper {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case ExtractSpatialJoinKeys(leftKey, rightKey, k, KNNJoin, left, right) =>
        simbaContext.sessionState.simbaConf.knnJoin match {
          case "RKJSpark" =>
      ...
      case ExtractSpatialJoinKeys(leftKey, rightKey, r, DistanceJoin, left, right) =>
        simbaContext.sessionState.simbaConf.distanceJoin match {
          case "RDJSpark" =>
