...
/**
  ...
  * (1) partitionted the query points and build a R-tree
  * (2) duplicate the data points with the built R-tree
  * (3) repatition the duplicated datapoints with the same data partition number as the query points
  * (4) nest-loop local computation
  */
...
case class RDJSpark(left_key: Expression, right_key: Expression, l: Literal,
  ...
  final val num_partitions = simbaSesstionState.simbaConf.joinPartitions
  final val sample_rate = simbaSesstionState.simbaConf.sampleRate
  final val max_entries_per_node = simbaSesstionState.simbaConf.maxEntriesPerNode
  final val transfer_threshold = simbaSesstionState.simbaConf.transferThreshold
