...
import org.apache.spark.sql.simba.util.ShapeType
...
private[simba] case class RTreeIndexedRelation(output: Seq[Attribute], child: SparkPlan, table_name: Option[String],
  ...
  private[simba] def buildIndex(): Unit = {
    val numShufflePartitions = sparkSession.sessionState.simbaConf.indexPartitions
    val maxEntriesPerNode = sparkSession.sessionState.simbaConf.maxEntriesPerNode
    val sampleRate = sparkSession.sessionState.simbaConf.sampleRate
    val transferThreshold = sparkSession.sessionState.simbaConf.transferThreshold
